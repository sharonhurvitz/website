...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function resetAllTracks() {
    tracks.forEach(track => {
      ...
      track.querySelector('.icon-play').style.display = 'block';
      track.querySelector('.icon-pause').style.display = 'none';
      track.querySelector('.progress-fill').style.width = '0%';
      track.querySelector('.track-time').textContent = '0:00';
    });
  }

  tracks.forEach(track => {
    const playBtn = track.querySelector('.play-toggle');
    const audioSrc = track.getAttribute('data-src');

    // Click anywhere on the track row to play/pause
    track.addEventListener('click', () => {
      const isPlaying = !audio.paused && currentTrackItem === track;
      ...
      if (isPlaying) {
        ...
        audio.play();
        track.querySelector('.icon-play').style.display = 'none';
        track.querySelector('.icon-pause').style.display = 'block';
      }
    });
  });
  ...
  audio.addEventListener('timeupdate', () => {
    ...
    const progressFill = currentTrackItem.querySelector('.progress-fill');
    const timeDisplay = currentTrackItem.querySelector('.track-time');
  ...
  audio.addEventListener('ended', resetAllTracks);
});
